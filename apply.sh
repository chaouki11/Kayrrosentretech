...
original_author="chaouki11"
original_name="kayrrentretientech"
original_urlname="KayrrEntretienTech"
original_description="Awesome kayrrentretientech created by chaouki11" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/kayrrentretientech "${name}"
...
then
    ...
    echo "then run 'kayrrentretientech' or 'python -m kayrrentretientech'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'kayrrentretientech' or 'python -m kayrrentretientech' or 'poetry run kayrrentretientech'"
fi
